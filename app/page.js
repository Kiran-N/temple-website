...
export default function Home() {
  ...
  useEffect(() => {
    // Load stats from localStorage (in real version, this comes from Firebase)
    loadStats();
    loadProgressPhotos();
  }, []);

  const loadStats = () => {
    // This will be replaced with Firebase real-time listener
  ...
  const loadProgressPhotos = () => {
    const photos = JSON.parse(localStorage.getItem('templeProgressPhotos') || '[]');
    setProgressPhotos(photos.slice(0, 6)); // Show latest 6 photos
  };
      ...
      <section className="container mx-auto px-4 py-16">
        ...
        <div className="grid md:grid-cols-4 gap-6 mb-16">
          <div className="bg-white rounded-xl shadow-lg p-6 text-center transform hover:scale-105 transition">
            ...
            <p className="text-3xl font-bold text-green-600">₹{stats.totalDonations.toLocaleString('en-IN')}</p>
          </div>

          <div className="bg-white rounded-xl shadow-lg p-6 text-center transform hover:scale-105 transition">
            <div className="text-4xl mb-2">📊</div>
            <h3 className="text-gray-600 font-semibold mb-2">Total Expenses</h3>
            <p className="text-3xl font-bold text-red-600">₹{stats.totalExpenses.toLocaleString('en-IN')}</p>
          </div>

          <div className="bg-white rounded-xl shadow-lg p-6 text-center transform hover:scale-105 transition">
            <div className="text-4xl mb-2">💵</div>
            <h3 className="text-gray-600 font-semibold mb-2">Current Balance</h3>
            <p className="text-3xl font-bold text-blue-600">₹{stats.balance.toLocaleString('en-IN')}</p>
          </div>

          <div className="bg-white rounded-xl shadow-lg p-6 text-center transform hover:scale-105 transition">
            <div className="text-4xl mb-2">🙏</div>
            <h3 className="text-gray-600 font-semibold mb-2">Generous Donors</h3>
            <p className="text-3xl font-bold text-purple-600">{stats.donorCount}</p>
          </div>
        </div>
          ...
          {progressPhotos.length > 0 ? (
            <div className="grid md:grid-cols-3 gap-6">
              {progressPhotos.map((photo, index) => (
                <div key={index} className="bg-white rounded-xl shadow-lg overflow-hidden transform hover:scale-105 transition">
...
function ExpenseBreakdown() {
  ...
  useEffect(() => {
    ...
    expenses.forEach(expense => {
      if (expense.category !== 'Facilitation Payments') { // Don't show bribes publicly
